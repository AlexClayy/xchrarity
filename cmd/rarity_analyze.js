...
let totalPunk = 1;
let traitTypeId = 1;
let traitDetailTypeId = 1;
let punkTraitTypeId = 1;
let punkScoreId = 1;
...
collectionData.forEach(element => {
    ...
    */
    ...
    element.attributes.forEach(attribute => {
        ...
        if (_.isEmpty(attribute.trait_type) || _.isEmpty(attribute.value) || attribute.value.toLowerCase() == 'none' || attribute.value.toLowerCase() == 'nothing' || attribute.value.toLowerCase() == '1') {
            return;
        }
        ...
        if (!traitTypeCount.hasOwnProperty(attribute.trait_type)) {
            ...
            insertTraitTypeStmt.run(traitTypeId, _.startCase(attribute.trait_type), traitDataType, 1);
            ...
            if (!ignoreTraits.includes(attribute.trait_type.toLowerCase())) {
                traitTypeCount[attribute.trait_type] = 1 + 1;
            } else {
        ...
        if (!traitDetailTypeCount.hasOwnProperty(attribute.trait_type+'|||'+attribute.value)) {
            insertTraitDetailTypeStmt.run(traitDetailTypeId, traitTypeIdMap[attribute.trait_type], attribute.value, 1);
            traitDetailTypeIdMap[attribute.trait_type+'|||'+attribute.value] = traitDetailTypeId;
            traitDetailTypeId = traitDetailTypeId + 1;
            if (!ignoreTraits.includes(attribute.trait_type.toLowerCase())) {
                traitDetailTypeCount[attribute.trait_type+'|||'+attribute.value] = 1 + 1;
            } else {
    ...
    if (!punkTraitTypeCount.hasOwnProperty(thisPunkTraitTypes.length)) {
        punkTraitTypeCount[thisPunkTraitTypes.length] = 1 + 1;
    } else {
    ...
    for(let traitType in traitTypeCount)
    {
        ...
        if (thisPunkTraitTypes.includes(traitType)) {
            ...
            if (!ignoreTraits.includes(traitType.toLowerCase())) {
                ...
                punkScore['trait_type_' + traitTypeId + '_percentile'] = 1;
                punkScore['trait_type_' + traitTypeId + '_rarity'] = 1;
